...
export default function CommandPalette({
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-start justify-center pt-[12vh] px-4">
      ...
      <div className="relative w-full max-w-xl bg-white border border-rule-strong rounded-none shadow-2xl overflow-hidden">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-rule-strong">
          ...
          <input
            ...
            autoComplete="off"
          />
          <kbd className="hidden sm:inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-paper-deep text-[10px] font-mono text-ink-mute border border-rule-strong">
            ESC
          </kbd>
        </div>
        ...
        <div className="max-h-[55vh] overflow-y-auto">
          ...
          {grouped.map((group) => (
            <div key={group.category}>
              <div className="px-4 pt-3 pb-1 text-[10px] uppercase tracking-wider font-bold text-ink-mute">
                {group.category}
              </div>
              {group.items.map((it) => {
                runningIndex += 1;
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`w-4 h-4 shrink-0 ${active ? 'text-vermillion' : 'text-ink-mute'}`}
                    />
                    <div className="min-w-0 flex-1">
                      <div className={`font-medium truncate ${active ? 'text-blue-900' : 'text-ink'}`}>
                        {it.label}
                      </div>
                      {it.hint && (
                        <div className="text-[11px] text-ink-mute truncate">{it.hint}</div>
                      )}
                    </div>
                    {active && <ArrowRight className="w-3.5 h-3.5 text-vermillion" />}
                  </button>
                );
              })}
            </div>
        ...
        <div className="px-4 py-2 border-t border-rule-strong bg-paper-deep flex items-center justify-between text-[10px] text-ink-mute">
          <span>
            <kbd className="px-1 py-0.5 rounded bg-white border border-rule-strong font-mono">↑↓</kbd>{' '}
            navigate ·{' '}
            <kbd className="px-1 py-0.5 rounded bg-white border border-rule-strong font-mono">↵</kbd>{' '}
            select
          </span>
          <span>
            <kbd className="px-1 py-0.5 rounded bg-white border border-rule-strong font-mono">⌘K</kbd>{' '}
            toggle
          </span>
        </div>
      </div>

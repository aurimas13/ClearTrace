...
const KYC_META = {
  ...
  tier_2: { label: 'Tier 2 — Standard DD', classes: 'bg-paper-deep text-vermillion border-rule-strong' },
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="min-w-0">
                ...
                <h3 className="text-base font-bold text-ink truncate">{profile.legalName}</h3>
              </div>
            </div>
            <p className="text-xs font-mono text-ink-soft truncate">{profile.account}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          {/* Risk header bar */}
          <div className="rounded-none border border-rule-strong p-4 bg-paper-deep/60">
          ...
          <Section icon={Globe2} title="Jurisdiction">
            <div className="text-sm text-ink font-semibold">
              ...
              <span className="text-xs text-ink-mute font-mono">({profile.jurisdiction.code})</span>
            </div>
            <p className="text-xs text-ink-soft mt-1">{profile.notes}</p>
          </Section>
          ...
          <Section icon={Crown} title="Beneficial owner chain">
            <ul className="space-y-1.5">
              {profile.beneficialOwners.map((b, i) => (
                <li
                  key={i}
                  className="flex items-center justify-between gap-2 px-3 py-2 rounded-md border border-rule-strong bg-white text-sm"
                >
                  <div className="min-w-0">
                    ...
                    <span className="text-[11px] text-ink-mute font-mono">{b.jurisdictionCode}</span>
                  </div>
                  <span className="text-sm font-bold text-ink-soft tabular-nums">{b.ownershipPct}%</span>
                </li>
              ))}
            </ul>
          </Section>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {involvedTxns.length === 0 && (
                  <tr>
                    <td colSpan={4} className="py-2 text-ink-mute italic text-center">
                ...
                {involvedTxns.map((t) => (
                  <tr key={t.id} className="border-t border-rule">
                    <td className="py-1.5 text-ink-soft whitespace-nowrap">
                      {new Date(t.transaction_date).toLocaleDateString()}
                    </td>
                    <td className="py-1.5 text-ink-soft font-medium">
                      {t.sender_account === account ? 'OUT' : 'IN'}
                    </td>
                    <td className="py-1.5 text-right font-semibold text-ink tabular-nums">
...
function Section({
  ...
  return (
    <div>
      <h4 className="text-[10px] uppercase tracking-wider font-bold text-ink-mute flex items-center gap-1.5 mb-2">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-rule-strong bg-white px-2 py-1.5">
